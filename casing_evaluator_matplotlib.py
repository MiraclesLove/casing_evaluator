    ...
    def onselect3(self, ymin, ymax):
        ...
        if ymin != ymax:
            ...
            if self.damage_Tag == 'Transformation':
                ...
                writer = pd.ExcelWriter('Transformation.xlsx', index=False)
                ...
                # 保存list到文件
                self.lines_temp = pd.DataFrame(self.lines, columns=['value'])
                writer = pd.ExcelWriter('Penetration.xlsx', index=False)
                self.lines_temp.to_excel(writer, 'Sheet1')
                writer.save()
                ...
                # 保存list到文件
                self.lines_temp = pd.DataFrame(self.lines, columns=['value'])
                writer = pd.ExcelWriter('Projection.xlsx', index=False)
                self.lines_temp.to_excel(writer, 'Sheet1')
                writer.save()
...
if __name__ == "__main__":
    fileDir = '.\\ning209H19-4_resample_jz.LAS'
    main = Matplot_class(fileDir)
    main.show()
